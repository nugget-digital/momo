...
lazy_static! {
    static ref CONFIG: Config = Config {
        ...
        static ref CLIENT: Mutex<Client> = Mutex::new(Client::new(&CONFIG).expect("Client::new"));
}

#[test]
fn lazily_creating_a_client() {
    // NOTE: lazy_static! silently initalizes the client on first access
    let client: MutexGuard<Client> =
        CLIENT.lock().expect("acquiring a mutex failed");
...
fn getting_balance() {
    ...
    let balance: Balance = client.get_balance().expect("Balance");
}
